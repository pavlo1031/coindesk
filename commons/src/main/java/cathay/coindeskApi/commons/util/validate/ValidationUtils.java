...
public class ValidationUtils {
	...
	private static <T, R> R __failsThen(T data, Function<?, ?> failsThen, Throwable cause, StackTraceElement[] stackTraceElements) throws RuntimeException {
		...
		if (returnValue instanceof RuntimeException) {
			...
			if (cause != null) {
				...
				// 手動重新建exception, 設定cause, stackTraceElement
	...
	/*package*/ static <T extends Throwable> T setStackTraceElements(T exception, StackTraceElement[] stackTraceElements) {
		if (stackTraceElements != null) {
			// 允許自行傳入exception過程
			stackTraceElements = stream(stackTraceElements)
								 .filter(FilterStackTraceElements)
			...
			//    (濾除: 太深/相關性不足的節點)
			exception.setStackTrace(stream(exception.getStackTrace())
									.filter(FilterStackTraceElements)
									.toArray(StackTraceElement[]::new));
		}
		return exception;
	...
	/*package*/ static <T extends Throwable> T setCause(T exception, Throwable cause) {		
		if (cause != null) {
			// 濾除: 太深/相關性不足的節點
			//       (濾除對象是「外部傳入的cause」的stackTraceElement)
			cause.setStackTrace(stream(cause.getStackTrace())
								.filter(FilterStackTraceElements)
								.toArray(StackTraceElement[]::new));
		}
		return exception;
