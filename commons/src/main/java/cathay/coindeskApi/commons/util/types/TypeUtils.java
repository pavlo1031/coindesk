...
import static cathay.coindeskApi.commons.util.MultiElementUtils.toList;
import static cathay.coindeskApi.commons.util.types.TypeUtils.Hoc.isArray;
import static cathay.coindeskApi.commons.util.types.TypeUtils.Hoc.mustBeType;
import static cathay.coindeskApi.commons.util.validate.Hoc.Validators.sizeGreaterThan;
...
public class TypeUtils {
	...
	public static class Hoc {
		...
		public static Predicate<Class<?>> isArray() {
			return (Class<?> c) -> c.isArray();
		}
		
		public static cathay.coindeskApi.commons.util.validate.Predicate<Class<?>> mustBeType(final Class<?> type) {
			return (Class<?> t) -> {
				return type.equals(t) || type.isAssignableFrom(t);
			};
		}
	}
	...
	public static boolean isVarargsWrapped(Object... values) {
		System.out.println("isVarargsWrapped(), values: " + Arrays.toString(values));
		...
		if (values.length == 1 && values[0] != null) {
			// 第1個元素的型別
			final Class firstElementType = values[0].getClass();
			if (firstElementType.isArray() && firstElementType.getComponentType().isPrimitive())
				return true;	
		}
		return false;
	}	
	
	public static <E> E[] flatWrappedVarargs(Object[] values) {
		Objects.requireNonNull(values, "The argument 'values' cannot be null.");
		if (!isVarargsWrapped(values))
			throw new IllegalArgumentException("參數'values'不是被錯誤地包裝的Object[]");
		
		Class<E> values0ArrayComponentType = (Class<E>) values[0].getClass().getComponentType();
		if (values0ArrayComponentType.isPrimitive())
			values0ArrayComponentType = toBoxed(values0ArrayComponentType);
		
		final List<?> list = toList(values[0]);
		final E[] arrayBuffer = (E[]) newInstance(values0ArrayComponentType, list.size());
		return (E[]) list.toArray(arrayBuffer);
	}
	...
	public static boolean containsMixedTypes(Object values) {
		Objects.requireNonNull(values, "The argument 'values' cannot be null.");	
		checkCondition(values.getClass(), isArray().or(mustBeType(Collection.class)), "Illegal 'values' type: The type of argument 'values' must be array or Collection");
		
		final Class valuesClass = values.getClass();
		final boolean isArray = valuesClass.isArray();
		final int length = (valuesClass.isArray())? Array.getLength(values) : ((Collection) values).size();
		if (length == 0)
			// 空集合: 要拋錯誤嗎？
			return false;
		
		Stream<?> stream = MultiElementUtils.stream(values);
		final Class<?> elementType = MultiElementUtils.get(values, 0).getClass();
		return stream.filter((c) -> !elementType.equals(c))
					  .findAny()
					  .isPresent();
	}
	...
	public static <T> Class<T> inferElementType(Object values, Class<T> defaultElemType) {
		...
		// 檢查: 型別
		// 必須為array或collection
		checkCondition(valuesType, or(Class::isArray, Collection.class::isAssignableFrom), "The type of argument 'values' is not supported, must be an array or a collection.");
		// 不支援map
		...
		// 檢查: 集合長度 (TODO: refactor)
		final int length = (valuesType.isArray())? Array.getLength(values) : ((Collection) values).size();
		if (length == 0) {
	 ...
	 */
	public static <K, V> Entry<Class<? extends K>, Class<? extends V>> inferMapEntryType(Map<?, ?> map) {
	...
	public static boolean isFloatingType(Object value) {
		final boolean valueIsNotClass = Class.class != value.getClass();
		final Class<?> type = (valueIsNotClass)? value.getClass() : (Class<?>) value;
		if (existsIn(type, double.class, float.class, Double.class, float.class, BigDecimal.class)) {
			return true;
		}
		return false;
	}
	...
	public static boolean isIntegralType(Object value) {
		final boolean valueIsNotClass = Class.class != value.getClass();
		final Class<?> type = (valueIsNotClass)? value.getClass() : (Class<?>) value;
		if (existsIn(type, byte.class, short.class, int.class, long.class,
	...
	public static <T> T[] toBoxedArray(Object array) {
		...
		if (elementType.isPrimitive())
			elementType = toBoxed(elementType);

		List<T> list = MultiElementUtils.toList(array);
		T[] arrayBuffer = (T[]) Array.newInstance(elementType, length);
		return list.toArray(arrayBuffer);
	}
