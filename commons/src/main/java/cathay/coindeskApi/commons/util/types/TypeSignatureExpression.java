...
import cathay.coindeskApi.commons.util.MapUtils;
		...
		if (matcher.matches() /* 實際執行匹配, 獲取結果 */) {
			...
			if (this.classname != null) {
				if (this.classname.contains("/"))
					...
					StringBuilder errorMessage = threadLocalBuffer.get();
					try {
						errorMessage.append("Illegal classname part ")
			...
			if (TypeDescriptorMappings.containsKey(this.typeDescriptor))
				...
				try {
					...
					// obtain buffer object from cache
					final StringBuilder errorMessage = threadLocalBuffer.get();
					
					// error log message
					System.out.println(errorMessage.append("[WARN] ").append("class not found: ").append('\"').append(this.classname).append('\"'));
					
					try {
						errorMessage.setLength(0);
						errorMessage.append("Failed to find the class: ").append('\"').append(this.classname).append('\"');
			...
			StringBuilder errorMessage = threadLocalBuffer.get();
			try {
				errorMessage.append("\"").append(s).append("\"")
							.append(" is not a correct type signature expression");
				throw new IllegalArgumentException(errorMessage.toString());
			} finally {
				errorMessage.setLength(0);
			}
