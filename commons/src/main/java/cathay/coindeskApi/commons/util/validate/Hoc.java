	...
	public static class Validators {
		...
		public static Predicate numberMatches(String variableName, Predicate<? extends Number> condition, String conditionDescription) {
			...
			if (isNoneEmpty(conditionDescription)) {
				buffer.append(": ").append('\'').append(conditionDescription).append('\'');
			} else {
				buffer.append('.');
			}
		...
		public static Predicate isDateBetween(final String variableName, final Date startDate, final Date endDate) {
			...
			StringBuilder buffer = threadLocalBuffer.get().append("日期時間參數")
														  ...
														  .append(" and ")
													      .append(DATE_FORMAT.format(endDate));
			try {
		...
		public static Predicate isDateBetween(final String variableName, final LocalDate startDate, final LocalDate endDate) {
			return isDateBetween(variableName, java.sql.Date.valueOf(startDate), java.sql.Date.valueOf(endDate));
		}
		
		public static Predicate isDateBetween(final LocalDateTime startDateTime, final LocalDateTime endDateTime) {
			return isDateBetween(null, startDateTime, endDateTime);
		}
		
		public static Predicate isDateBetween(final String variableName, final LocalDateTime startDateTime, final LocalDateTime endDateTime) {
			...
			return isDateBetween(variableName,
		...
		public static Predicate isDateBefore(String variableName, final Date before, final boolean inclusive) {
			...
			StringBuilder buffer = threadLocalBuffer.get().append("日期時間參數")
														  .append(isNoneEmpty(variableName) ? "%s" : "")
														  .append(", 必須早於").append(DATE_FORMAT.format(before));
			try {
				return doReturnHocFlow(variableName, buffer.toString(), (Date date) -> {
					if (inclusive)
		...
		public static Predicate isDateAfter(String variableName, final Date after, final boolean inclusive) {
			...
			StringBuilder buffer = threadLocalBuffer.get().append("日期時間參數")
														  .append(isNoneEmpty(variableName) ? "%s" : "")
														  .append(", 必須晚於").append(DATE_FORMAT.format(after));
			try {
			...
			try {
				return doReturnHocFlow(variableName, buffer.toString(), (T argument) -> {
					...
					if (argumentClass.isArray())
						return Array.getLength(argument) == size;	
					if (Collection.class.isAssignableFrom(argumentClass))
						return ((Collection) argument).size() == size;
					if (Map.class.isAssignableFrom(argumentClass))
						return ((Map) argument).size() == size;
		...
		public static <T> Predicate<T> sizeGreaterThan(String variableName, int minSizeExlusive) {
			...
			try {
				return doReturnHocFlow(variableName, buffer.toString(), (T argument) -> {
					...
					final Class<?> argumentClass = argument.getClass();
					if (argumentClass.isArray())
						return Array.getLength(argument) > minSizeExlusive;	
					if (Collection.class.isAssignableFrom(argumentClass))
						return ((Collection) argument).size() > minSizeExlusive;
					if (Map.class.isAssignableFrom(argumentClass))
						return ((Map) argument).size() > minSizeExlusive;
		...
		public static Predicate containsKey(String variableName, Object key) {
			StringBuilder buffer = threadLocalBuffer.get().append("The map ")
														  .append(isNoneEmpty(variableName) ? "%s " : "")
														  .append("must have the key ").append('\'').append(key).append('\'');
			try {
